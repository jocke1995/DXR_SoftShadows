...
TAARenderTask::TAARenderTask(ID3D12Device5* device, RootSignature* rootSignature, const std::wstring& VSName, const std::wstring& PSName, std::vector<D3D12_GRAPHICS_PIPELINE_STATE_DESC*>* gpsds, const std::wstring& psoName)
...
TAARenderTask::~TAARenderTask()
{
	delete m_CbPerObjectUploadResource;
}

void TAARenderTask::SetCommandInterface(CommandInterface* inter)
{
	m_pCommandInterfaceTemp = inter;
}

void TAARenderTask::SetFullScreenQuadMesh(Mesh* fsq)
{
	m_pFullScreenQuadMesh = fsq;
}

void TAARenderTask::CreateSlotInfo()
{
	// Mesh
...
void TAARenderTask::Execute()
{
	...
	commandList->SetGraphicsRootDescriptorTable(RS::dtRaytracing, m_DescriptorHeaps[DESCRIPTOR_HEAP_TYPE::CBV_UAV_SRV]->GetGPUHeapAt(m_pCurrFrameUAVIndex));
	commandList->SetGraphicsRootDescriptorTable(RS::dtUAV, m_DescriptorHeaps[DESCRIPTOR_HEAP_TYPE::CBV_UAV_SRV]->GetGPUHeapAt(m_pTAAPingPong->GetUAV()->GetDescriptorHeapIndex()));
...
void TAARenderTask::SetTAAPingPongResource(PingPongResource* TAAPingPong)
{
	m_pTAAPingPong = TAAPingPong;
}

void TAARenderTask::SetCurrFrameUAVIndex(unsigned int index)
{
	m_pCurrFrameUAVIndex = index;
}
